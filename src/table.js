...
export const createTable = (columnsArray, dataArray, tableId) => {
  if (
    ...
    throw new Error(
      "Para a execução correta, precisamos de um array com as colunas, outro com as infos das linhas e também o id do elemento tabela selecionado "
    );
  }
...
function createTableHeader(tableReference, columnsArray) {
  ...
  ["bg-zinc-200", "text-Slate-200", "text-black", "sticky", "top-0"].forEach(
...
function createTableBody(tableReference, tableItems, columnsArray) {
  ...
  for (const [itemIndex, tableItem] of tableItems.entries()) {
    ...
    if (itemIndex % 2 !== 0) {
      tableRow.classList.add("bg-stone-100", "text-black");
    }
